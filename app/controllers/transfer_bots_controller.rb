class TransferBotsController < ApplicationController
  ...
  def transfer_bot_params
    params.require(:transfer_bot).permit(:worker_class, :parse_config_id , :description, :enabled, :schedule, :remote_path, :patterns, :source_delete_after, ftp_config_attributes: [:host, :port, :user, :secret, :passive, :id]).merge(owner: current_owner, worker_class: 'FileDownloadWorker')
  end
